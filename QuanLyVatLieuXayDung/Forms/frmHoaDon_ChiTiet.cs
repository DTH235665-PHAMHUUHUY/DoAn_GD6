...
namespace QuanLyVatLieuXayDung.Forms
{
    public partial class frmHoaDon_ChiTiet : Form
    {
        ...
        {
            var dsVatLieu = context.VatLieu.ToList();
            cboVatLieu.DisplayMember = "TenVatLieu";
            cboVatLieu.ValueMember = "ID";
            cboVatLieu.DataSource = dsVatLieu;
        }
        ...
        private void frmHoaDon_ChiTiet_Load(object sender, EventArgs e)
        {
            ...
            if (id != 0) // Đã tồn tại chi tiết
            {
                ...
                var ct = context.HoaDon_ChiTiet.Where(r => r.HoaDonID == id).Select(r => new DanhSachHoaDon_ChiTiet
                {
                    ...
                    TenVatLieu = r.VatLieu.TenVatLieu,
        ...
        {
            if (string.IsNullOrWhiteSpace(cboVatLieu.Text))
                MessageBox.Show("Vui lòng chọn Vật liệu.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            else
            {
                ...
                else // Thêm mới
                {
                    ...
                    foreach (var item in hoaDonChiTiet.ToList())
                    {
                        ...
                        // THÊM DÒNG NÀY: Gán trực tiếp đối tượng Vật Liệu để EF Core tự lo phần khóa ngoại
                        ct.VatLieu = context.VatLieu.Find(item.SanPhamID);
        ...
        private void cboVatLieu_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboVatLieu.SelectedValue != null && int.TryParse(cboVatLieu.SelectedValue.ToString(), out int maSanPham))
            {
